...
type KeyLevelsData = {
    ...
    pricePosition: {
        ...
        percentInRange: number; // 0% = at PDL, 100% = at PDH
...
function getActiveSession(): string {
    ...
    // Check which session is active (with overlap handling)
    if (hour >= 0 && hour < 9) return 'asian';
    if (hour >= 8 && hour < 16) return 'london';
    if (hour >= 13 && hour < 22) return 'newYork';

    // Default based on closest session
...
export async function GET() {
    try {
        ...
        console.error('Key levels API error:', error);
