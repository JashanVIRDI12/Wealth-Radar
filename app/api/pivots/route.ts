...
function calculatePivots(high: number, low: number, close: number) {
    ...
    return {
        ...
        r1: Number((2 * pp - low).toFixed(3)),
        r2: Number((pp + (high - low)).toFixed(3)),
        r3: Number((high + 2 * (pp - low)).toFixed(3)),
        s1: Number((2 * pp - high).toFixed(3)),
        s2: Number((pp - (high - low)).toFixed(3)),
        s3: Number((low - 2 * (high - pp)).toFixed(3)),
    };
}
...
function getVolatilityLevel(atr: number, price: number): 'low' | 'medium' | 'high' {
    ...
    // For USD/JPY, typical ATR ranges:
    // Low: < 0.5% of price
    // Medium: 0.5-1% of price
    // High: > 1% of price
...
export async function GET() {
    try {
        ...
        const pivotData: PivotData = {
            dailyOHLC: {
                open: Number(today.open.toFixed(3)),
                high: Number(today.high.toFixed(3)),
                low: Number(today.low.toFixed(3)),
                close: Number(today.close.toFixed(3)),
                date: today.date,
            },
            pivots,
            atr: {
            ...
            pricePosition,
            rangeAnalysis: {
                todayHigh: Number(today.high.toFixed(3)),
                todayLow: Number(today.low.toFixed(3)),
                currentRange: Number(todayRange.toFixed(3)),
                atrPercent: Number(atrPercent.toFixed(1)),
                rangeRemaining: Number(rangeRemaining.toFixed(3)),
            },
