        ...
        <div className="space-y-3">
            ...
            <div className="p-4 rounded-xl bg-black/20 border border-white/5">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <div className="text-lg font-semibold text-white tracking-tight">{getPhaseLabel(data.marketPhase)}</div>
                    </div>
                    <div className="text-right">
                        <div className="text-2xl font-semibold text-white tabular-nums">
                            {time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                        </div>
                        <div className="kicker">Local Time</div>
                    </div>
                </div>
            </div>
                    ...
                    >
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        {session.isActive && (
                                            <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/70 text-xs font-medium">
                                                ACTIVE
                                            </span>
                                        )}
                                        {session.overlap && (
                                            <span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/70 text-xs">
                                                + {session.overlap}
                                            </span>
                                        )}
                                    </div>
                            ...
                            {session.isActive && session.timeRemaining && (
                                <div className="text-right">
                                    <div className="text-sm font-medium text-white/80 tabular-nums">{session.timeRemaining}</div>
            ...
            <div className="p-3 rounded-xl bg-black/20 border border-white/5">
                <p className="text-xs text-zinc-500 flex items-center gap-1.5">
                    ...
                    <span className="text-white/60">Best USD/JPY volatility:</span> Tokyo open, London-NY overlap
                </p>
            </div>
        </div>
    );
